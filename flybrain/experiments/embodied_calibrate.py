...
def _viewer(fly_seed: int, calibration: Calibration | None = None, tethered: bool = False):
    ...
    fly = EmbodiedFly(vision=EMBODIED_VISION, scene=SceneConfig(), seed=fly_seed,
                      tether=TetherConfig() if tethered else None)
...
def observe_chunk(indices: list[int], fly_seed: int, tethered: bool = False) -> dict[str, np.ndarray]:
    from flybrain.experiments.calibrate import CAL_SEED, make_post

    viewer = _viewer(fly_seed, tethered=tethered)
...
def look_chunk(indices: list[int], fly_seed: int, calibration: dict, homeostasis: bool = False,
               tethered: bool = False) -> list[dict]:
    from flybrain.experiments.calibrate import TEST_SEED, make_post

    viewer = _viewer(fly_seed, Calibration(**calibration), tethered=tethered)
...
def record(n_posts: int, workers: int, tethered: bool = False) -> str:
    ...
    path = RUNS / f"embodied-cal{'-bagli' if tethered else ''}-{time.strftime('%Y%m%d-%H%M%S')}.npz"
...
def fit(data: dict, confirmed: bool, tethered: bool = False) -> Calibration:
    ...
    return Calibration.fit(rates, floor, visible(data["body"], tethered) if confirmed else None)
...
def report(data: dict, cal: Calibration, plain: Calibration, tethered: bool = False) -> dict:
    ...
    cons = consistency(data, plain, tethered)
    print("onaysız kuralda görünür hareketli karar:",
          {c: (f"%{100 * r['gorunur']:.0f} / {r['karar']}" if r["karar"] else "–") for c, r in cons.items()})
    return cons


def validate(cal: Calibration, n_posts: int, workers: int, tethered: bool = False) -> dict:
    t0 = time.perf_counter()
    chunks = _chunks(n_posts, workers)
    results = {}
    with ProcessPoolExecutor(workers) as pool:
        for rep in (0, 1):
            ...
            parts = pool.map(look_chunk, chunks, seeds, [cal.__dict__] * len(chunks),
                             [False] * len(chunks), [tethered] * len(chunks))
...
def session(cal: Calibration, flies: int, n_posts: int, tethered: bool = False) -> dict:
    ...
    with ProcessPoolExecutor(flies) as pool:
        sessions = list(pool.map(look_chunk, indices, [8000 + i for i in range(flies)],
                                 [cal.__dict__] * flies, [True] * flies, [tethered] * flies))
...
def main() -> None:
    ...
    ap.add_argument("--save", action="store_true", help="eşikleri kalibrasyon dosyasına yaz")
    ap.add_argument("--bagli", action="store_true",
                    help=f"sinek ekrana bağlı (K-040); eşikler {CALIBRATION_TETHERED_PATH.name} dosyasına yazılır")
    ...
    bagli = args.bagli
    hedef = CALIBRATION_TETHERED_PATH if bagli else CALIBRATION_EMBODIED_PATH
    path = args.fit_from or record(args.posts, args.workers, bagli)
    data = dict(np.load(path))
    cal = fit(data, confirmed=True, tethered=bagli)
    plain = fit(data, confirmed=False, tethered=bagli)
    out = {"kayit": path, "bagli": bagli, "kalibrasyon": cal.__dict__,
           "uyum_onaysiz": report(data, cal, plain, bagli)}
    if args.save:
        cal.save(hedef)
        print(f"kalibrasyon kaydedildi: {hedef}")
    if args.test_posts:
        out["dogrulama"] = validate(cal, args.test_posts, args.workers, bagli)
    if args.session:
        out["oturum"] = session(cal, args.flies, args.session, bagli)
    rapor = RUNS / f"embodied-cal-rapor{'-bagli' if bagli else ''}-{time.strftime('%Y%m%d-%H%M%S')}.json"
