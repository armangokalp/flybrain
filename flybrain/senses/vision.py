...
class VisionEncoder:
    def __init__(self, conn: Connectome, config: VisionConfig = VisionConfig(), eye: Eye | None = None):
        if config.mode not in MODES:
            raise ValueError(f"bilinmeyen yöntem: {config.mode}")
        self.conn = conn
        self.cfg = config
        ...
        # (grup adı, nöronlar, kanal, kutup): kutup +1 aydınlığa, −1 karanlığa yanıt
        groups: list[tuple[str, np.ndarray, str, int]] = []
        tonic: list[np.ndarray] = []
        if config.mode in ("off", "onoff"):
            groups.append(("L2+L3", sel(type=["L2", "L3"]), "lum", -1))
        if config.mode == "onoff":
            groups.append(("Mi1+Tm3", sel(type=["Mi1", "Tm3"]), "lum", +1))
        if config.mode == "foto":
            groups += [
                ("R1-R6", S["R1_R6"], "lum", +1),
                ("R7", S["R7"], "blue", +1),
                ("R8p", sel(type="R8p"), "blue", +1),
                ("R8y", sel(type=["R8y", "R8d", "R8_unclear"]), "green", +1),
            ]
            tonic = [sel(type=["L1", "L2", "L3", "Mi1", "Tm3"])]

        self.groups = []
        for name, idx, channel, polarity in groups:
    ...
    def encode(self, img: np.ndarray) -> Stimulus:
        contrast = self.column_contrast(img)
        stim = Stimulus.empty()
        for name, idx, _, _, _, polarity in self.groups:
            c = contrast[name] * polarity
            if self.cfg.mode == "foto":
                hz = self.cfg.r_base_hz * np.clip(1.0 + c, 0.0, 1.0 + CONTRAST_SAT)
            else:
                hz = self.cfg.r_max_hz * np.clip(c / CONTRAST_SAT, 0.0, 1.0)
